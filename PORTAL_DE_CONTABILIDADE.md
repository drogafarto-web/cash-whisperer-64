...
graph TD
    ...
    B --> C[Função de backend gera token]
...
```mermaid
graph TD
    A[Tarefa agendada executa dia 5] --> B[Verifica dados do mês anterior]
    B --> C{Dados completos?}
...
O lembrete mensal é executado via tarefa agendada no banco de dados:
...
- Políticas de segurança RLS protegem dados internos
...
3. Verifique logs da função de backend `save-accounting-data`
...
| `SUPABASE_URL` | URL do projeto de backend |
| `SUPABASE_ANON_KEY` | Chave anônima do backend |
| `SUPABASE_SERVICE_ROLE_KEY` | Chave de serviço (para funções de backend) |
